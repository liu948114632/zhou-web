...
$.fn.Title = function (a) {
    ...
    var c = $(this).offset();
    if (a.IsShow) {
        $(this).MyTitle({defaultCss: "tipsr", title: zb.L("ZB提示您"), trigger: "manual"});
        if (!$(this).attr("errmsg")) {
                ...
                if (a[d].tagName.toLowerCase() == "select") {
                    ...
                    if (a[d].getAttribute("pattern") != null && a[d].getAttribute("pattern") != undefined && c <= 0) {
                        if (!CheckSelect(a[d])) {
                            if ($(a[d]).attr("errorName")) {
                                currentErrorObj = a[d];
                                Wrong("请选择" + $(a[d]).attr("errorName") + "！", {callback: "errSelectCalback(" + e + ")"})
                            } else {
                                if (e) {
...
zb.list = {
    basePath: "", funcName: "", isInit: false, ui: function (c) {
        ...
        var g = f.id, c = f.width || 560, e = f.height || 360, d = f.title || "添加/编辑" + this.funcName,
            a = f.scroll || "auto";
        ...
        var d = a.id, c = this;
        Ask2({
            Title: zb.L("确定删除本项吗？"), call: function () {
                zb.ajax({
